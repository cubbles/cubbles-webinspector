...
(function () {
  var cubblesInfo = {};
  window.runtimeId = undefined;
  var lastSetCubbleSlotsInfoRuntimeId;
  const COL_CLASSES = [
  ...
  function initCubblesInfo (slotsInfo) {
    ...
    cubbleInfo.memberId = slotsInfo.memberId;
  ...
  function updateSidebar (slotsInfo) {
    if (slotsInfo.runtimeId !== lastSetCubbleSlotsInfoRuntimeId) {
      window.runtimeId = slotsInfo.runtimeId;
      fillSlotsTables(slotsInfo);
    } else {
      updateSlotsValues(slotsInfo);
    }
    lastSetCubbleSlotsInfoRuntimeId = slotsInfo.runtimeId;
    sendMessageToInjectedScript({name: 'slot-info-was-updated', content: lastSetCubbleSlotsInfoRuntimeId });
  }
  ...
  function requestSlotsInfo (targetRuntimeId) {
    targetRuntimeId = targetRuntimeId || window.runtimeId;
  ...
  function setSlotInfoInCubblesInfo (slotInfo, property, value, targetRuntimeId) {
    ...
    cubblesInfo[targetRuntimeId || window.runtimeId][key][slotInfo.slotId][property] = value;
  }

  function getSlotInfoInCubblesInfo (slotInfo, property, targetRuntimeId) {
    let key = slotInfo.direction === 'input' ? 'inputSlots': 'outputSlots';
    return cubblesInfo[targetRuntimeId || window.runtimeId][key][slotInfo.slotId][property];
  }
  ...
  function createSlotsInfoElementsInTables (slots, table) {
    ...
    function createSetValueBtn(slotInfo) {
      ...
      slotVBtn.addEventListener('click', function (e) { 
        postSetSlotMsg(e);
        var baseValueRBSelector = `[data-slot-id="${slotInfo.slotId}"][data-slot-direction="${slotInfo.direction}"][data-base-value-key="value"]`;
        document.querySelector(baseValueRBSelector).checked = true;
      });
      return slotVBtn;
    }
  ...
  function createBaseCodeRB (slotInfo, type, autosetCB, checked) {
    ...
    if ((type === 'init' || type === 'first') &&
      getSlotInfoInCubblesInfo(slotInfo, baseValueKey) === undefined) {
  ...
  function setCubbleDepTree(runtimeId) {
    ...
    if (depTreeViewer.setDepTree) {
      ...
      document.addEventListener('cifReady', setCubbleDepTreeAux);
    }
    function setCubbleDepTreeAux() {
  ...
  function highlightMemberInComponentViewer (runtimeId) {
    if (cubblesInfo.hasOwnProperty(runtimeId) && cubblesInfo[runtimeId].memberId) {
      let componentVwr = document.querySelector('cubx-generic-component-viewer');
      componentVwr.setHighlightedMember(cubblesInfo[runtimeId].memberId);
    }
  }
  ...
  function initCubblesTree() {
    $('#cubblesTree').on("changed.jstree", function (e, data) {
      if (data.hasOwnProperty('node')) {
        ...
        highlightMemberInComponentViewer(data.node.id);
      }
    });
  ...
  function selectDefaultCubble () {
    ...
    window.runtimeId = instance.settings.core.data[0].id;
  }
