...
(function () {
  ...
  window.setTagInitInfo = function (tagInitInfo) {
    if (tagInitInfo.hasOwnProperty('slotInits')) {
      let codeContainer = document.querySelector('#coreInitsContainer > pre');
      setJsonToSlotValueElement(codeContainer, tagInitInfo.slotInits);
    }
    if (tagInitInfo.hasOwnProperty('connectionsInfo')) {
      let codeContainer = document.querySelector('#coreConnectionsContainer > pre');
      setJsonToSlotValueElement(codeContainer, tagInitInfo.connectionsInfo);
    }
  }
  ...
  function fillSlotsTables(slotsInfo) {
    ...
    infos.forEach(function(info, i) {
      ...
      if (slotsInfo.hasOwnProperty(info.slotInfoKey) && slotsInfo[info.slotInfoKey].length > 0 ) {
        tableContainer.querySelector('.no-slots-message').classList.add('hidden');
        ...
        tableContainer.querySelector('.no-slots-message').classList.remove('hidden');
  ...
  function updateTagInitInfo (targetRuntimeId) {
    chrome.devtools.inspectedWindow.eval(`window.postMessageToInjectedScript({ name: 'get-tag-init-info', content: '${targetRuntimeId}' });`,
      { useContentScriptContext: true });
  }
  ...
  function initCubblesTree() {
    $('#cubblesTree').on("changed.jstree", function (e, data) {
      if (data.hasOwnProperty('node')) {
        ...
        updateTagInitInfo(data.node.id);
      }
    });
