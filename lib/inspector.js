...
(function () {
  ...
  function createElementForUseCurrent (slotInfo) {
    let container = document.createElement('div');
    container.appendChild(createBaseCodeRB(slotInfo, 'current', true));
    container.appendChild(createBaseCodeRB(slotInfo, 'init'));
    container.appendChild(createBaseCodeRB(slotInfo, 'first'));
    return container;
  }

  function createBaseCodeRB (slotInfo, type, checked) {
    let labelText;
    let title;
    let baseValueKey;
    switch (type) {
      ...
      case 'init':
        ...
        break;
      case 'first':
        labelText = 'First available Value';
    ...
    useOptionRB.addEventListener('change', handleUseBaseValueRB);
  ...
  function handleUseBaseValueRB(e) {
    if (this.checked) {
      var slotInfo = {
        slotId: this.getAttribute('data-slot-id'),
        direction: this.getAttribute('data-slot-direction'),
        type: this.getAttribute('data-slot-type'),
      }
      var baseValueKey = this.getAttribute('data-base-value-key');
      updateSlotValueElement(slotInfo, getSlotInfoInCubblesInfo(slotInfo, baseValueKey));
      console.log(getSlotInfoInCubblesInfo(slotInfo, baseValueKey), cubblesInfo, slotInfo, baseValueKey);
    }
  }
