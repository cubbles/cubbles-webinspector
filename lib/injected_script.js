(function () {
    ...
    if (cifIsReady()) {
        switch (e.detail.name) {
            ...
            case 'get-slots-info':
                ...
                break;
            case 'get-tag-init-info':
                handleTagInitInfoRequest(e.detail.content);
                break;
  ...
  function handleTagInitInfoRequest (runtimeId) {
    if (runtimeId === 'no-cubbles') {
      postMessage('set-tag-init-info', runtimeId);
      lastSentTagInitInfo = null;
    } else {
      if (!lastSentTagInitInfo || lastSentTagInitInfo.runtimeId !== runtimeId) {
        var tagInitInfo = getTagInitInfo(document.querySelector(`[runtime-id="${runtimeId}"]`), runtimeId);  
        postMessage('set-tag-init-info', tagInitInfo);
        lastSentTagInitInfo = tagInitInfo;
      }
    }
  }

  function getTagInitInfo (cubble, runtimeId) {
    let tagInitInfo = {};
    tagInitInfo.runtimeId = runtimeId;
    tagInitInfo.slotInits = extractSlotInitInfo(cubble);
    tagInitInfo.connectionsInfo = extractConnectionsInfo(cubble);
    return tagInitInfo;
  }
  ...
  function extractSlotInitInfo (cubble) {
    ...
    if (coreInit) {
      ...
      slotInits.forEach(function(slotInit) {
        let slotInitInfo = {};
        slotInitInfo.slotId = slotInit.getAttribute('slot');
        slotInitInfo.slotValue = slotInit.innerText;
        slotsInitInfo.push(slotInitInfo);
      });
    }
  ...
  function getCubbles () {
    return document.querySelectorAll('[runtime-id][cubx-webpackage-id]');
  }
