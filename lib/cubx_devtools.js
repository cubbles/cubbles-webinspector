...
(function () {
        ...
        switch (message.name) {
          ...
          case 'set-dep-tree':
            if (cubblesPanelWindow) {
              cubblesPanelWindow.currentDepTree = message.content;
            }
            break;
          case 'cif-ready':
            if (cubblesPanelWindow) {
  ...
  function createCubblesInspectorPanels () {
      ...
      function (panel) {
        panel.onShown.addListener(function (extPanelWin) {
          if (firstTimeCubblesPanel) {
            ...
            depTreeVwr = extPanelWin.document.getElementById('rootDepsTree');
            ...
            extPanelWin.document.addEventListener('cifReady', function () {
              ...
              extPanelWin.addOnShownListener('depsTreeViewerT', depTreeVwr, true, function () {
                depTreeVwr.setDepTree(cubblesPanelWindow.currentDepTree);
                cubblesPanelWindow.handleDepTreesAutoScale(depTreeVwr);
              });
              requestPanelInformation(true);
              forceRequestSlotsInfo();
    ...
    function forceRequestSlotsInfo (){
      if (cubblesPanelWindow.runtimeId) {
        postMessage('get-slots-info-force', cubblesPanelWindow.runtimeId);
      }
    }
